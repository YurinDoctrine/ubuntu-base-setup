...
# Before hop in
sudo apt update &&
    sudo apt install -f --assume-yes 9base base-files binutils curl fonts-terminus git gnupg2 haveged kmod libinput-dev pkgconf psmisc software-properties-common ufw zstd wget xdg-utils &&
