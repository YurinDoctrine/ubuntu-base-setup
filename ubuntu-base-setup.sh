...
echo -e "WL_OUTPUT_SUBPIXEL_NONE=none
...
sudo sed -i -e 's| defaults| rw,lazytime,relatime,discard=async,commit=600|g' /etc/fstab
sudo sed -i -e 's| errors=remount-ro| rw,lazytime,relatime,discard=async,commit=600,errors=remount-ro|g' /etc/fstab
...
tmpfs /tmp tmpfs rbind,nodiratime,nodev,nosuid 0 0" | sudo tee -a /etc/fstab
